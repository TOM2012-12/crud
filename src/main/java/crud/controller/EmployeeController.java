...
//        想要连续显示5页，就加上参数5即可
    ...
    @ResponseBody
    public PageInfo getEmployee(String empName,@RequestParam(value = "pn",defaultValue = "1")Integer pn){
        PageHelper.startPage(pn,5);
        List<Employee> emps=employeeService.selectbyName(empName);
        PageInfo pageInfo=new PageInfo(emps,5);
        return pageInfo;
    }
    ...
    @ResponseBody
    @RequestMapping(value = "/emp1/{empId}",method = RequestMethod.PUT)
    public Msg updateEmp1(Employee employee, @PathVariable String empId){
     ...
     */
    @ResponseBody
    @RequestMapping(value = "/emp1/{ids}",method = RequestMethod.DELETE)
    public Msg deleteEmpById1(@PathVariable("ids") String ids){
        if(ids.contains("-")){
            List<Integer> del_ids=new ArrayList<>();
            String str_ids[]=ids.split("-");
